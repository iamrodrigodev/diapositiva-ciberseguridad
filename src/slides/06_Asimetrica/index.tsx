    ...
    <SlideLayout>
      ...
      <div className="flex gap-6 flex-1 overflow-hidden mt-2">
        ...
        <motion.div variants={fadeUp} className="w-[45%] flex flex-col gap-3">
          <div className="rounded-3xl p-7 flex-1 flex flex-col gap-4 relative overflow-hidden"
            ...
            <h3 className="font-bold text-[22px] bg-linear-to-r from-blue-400 to-indigo-400 bg-clip-text text-transparent">ECC — Curvas Elípticas</h3>
            ...
            <div className="flex flex-col gap-4 mt-1">
              <div className="flex gap-3 items-start">
                ...
                <div>
                  <h4 className="text-white font-bold text-[15px] mb-0.5">Geometría en lugar de Primos</h4>
                  <p className="text-white/65 text-[14px] leading-snug">
              ...
              <div className="flex gap-3 items-start">
                ...
                <div>
                  <h4 className="text-white font-bold text-[15px] mb-0.5">Magia Unidireccional (ECDLP)</h4>
                  <p className="text-white/65 text-[14px] leading-snug">
                    Calcular los rebotes hacia adelante es fácil, pero adivinar el punto de origen (tu clave privada) es matemáticamente imposible.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 mt-auto pt-3">
              {[['RSA-2048', '2048 bits', '#a78bfa', 'rgba(167,139,250,0.06)', 'rgba(167,139,250,0.2)'], ['ECC-256', '256 bits', '#60a5fa', 'rgba(96,165,250,0.06)', 'rgba(96,165,250,0.2)']].map(([name, bits, color, bg, border]) => (
                <div key={name as string} className="rounded-2xl px-5 py-4 transition-all hover:scale-[1.02]"
                  style={{ background: bg as string, border: `1px solid ${border}` }}>
                  <div className="text-[17px] font-bold" style={{ color: color as string }}>{name as string}</div>
                  <div className="text-white/60 text-[14px] mt-1 font-mono">{bits as string} de clave</div>
                  <div className="text-[10px] font-bold text-white/40 uppercase tracking-widest mt-2.5">seguridad eq.</div>
                </div>
              ))}
            </div>

            <div className="flex items-center gap-4 bg-white/5 border border-white/10 rounded-2xl p-4 mt-1">
              <Cpu size={28} className="text-white/30 shrink-0" />
