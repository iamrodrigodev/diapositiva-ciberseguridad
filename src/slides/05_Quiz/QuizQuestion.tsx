    ...
    <div className="flex flex-col gap-4 flex-1">
      ...
      <div className="grid grid-cols-2 gap-3">
        {question.opts.map((opt, i) => {
          ...
          let border = 'rgba(255,255,255,0.08)', bg = 'rgba(255,255,255,0.03)', tc = 'rgba(255,255,255,0.6)'
          if (revealed && isCorrect)              { border = '#34d399'; bg = 'rgba(52,211,153,0.12)';  tc = '#34d399' }
          ...
          return (
            <button key={i} onClick={() => onVote(i)} disabled={revealed}
              className="relative rounded-xl px-4 py-3 text-left text-sm font-medium transition-all duration-200 overflow-hidden disabled:cursor-default"
              ...
              <div className="relative flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <span className="font-mono text-[10px] font-bold" style={{ color: optColor }}>
                ...
                {revealed && (
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    {isSelected && !isCorrect && <XCircle size={13} className="text-red-400" />}
      ...
      <div className="flex gap-3 mt-auto">
        {!revealed
          ? <Button variant="primary" className="flex-1" onClick={onReveal} disabled={selected === null}>Revelar respuesta</Button>
          : !isLast
            ? <Button variant="secondary" className="flex-1" onClick={onNext}>Siguiente →</Button>
            : <div className="flex-1 text-center font-mono text-sm text-emerald-400 py-2.5">
